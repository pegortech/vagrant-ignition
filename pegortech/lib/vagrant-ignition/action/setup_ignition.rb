    ...
    module Action
      class SetupIgnition
        ...
        def call(env)
          ...
          env[:machine].ui.info "Configuring Ignition Config Drive for attachment to #{controller}"
          ...
          env[:machine].provider.driver.execute("storageattach", "#{env[:machine].id}", "--storagectl", "#{controller}", "--device", "0", "--port", "1", "--type", "hdd", "--medium", "#{File.join(drive_root, (drive_name + ".vmdk"))}")
